...
public:
	 ...
	 * @param pin The pin OF YOUR ARDUINO (not the PCF8575) to use as "INT" pin for interrupts detection
	 ...
	 * @remarks If multiple PCF8575 are wired on the same "INT" pin this function should be called only one time
	 */
	 ...
	 * 
	 * @remarks Call this routine from your wrapping routine to detect and process interrupts (if any) of this PCF8575 instance.
	 */
	void checkForInterrupt();

	/**
	 * Attach a function to an interrupt event of a pin of the PCF8575
	 * 
	 ...
	 * @remarks 1 PCF8575 pin = 1 interrupt, multiple interrupts on the same pin is not supported
	 */
